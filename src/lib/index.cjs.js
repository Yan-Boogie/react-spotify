...
const UserTokenContext = f.createContext('User-Token-Context');
...
function useSpotifyApiRequest({ requestUrl, requestMethod, }) {
    const [isLoading, setLoading] = f.useState(false);
    const [error, setError] = f.useState(null);
    const [apiReturned, setApiReturned] = f.useState(null);
    const token = f.useContext(UserTokenContext);
    f.useEffect(() => {
        let canceled = false;
        setLoading(true);
        try {
...
function useSpotify(token = TOKEN) {
    return {
        Spotify: ({ children }) => (jsxRuntime.jsx(UserTokenContext.Provider, Object.assign({ value: token }, { children: () => children() }), void 0)),
    };
}

exports.useSpotify = useSpotify;
exports.useSpotifyApiRequest = useSpotifyApiRequest;
